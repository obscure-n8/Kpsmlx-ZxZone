...
from bot.helper.themes import zxzone_minimal

AVL_THEMES = {}
for theme in listdir('bot/helper/themes'):
    if theme.startswith('zxzone_') and theme.endswith('.py'):
        AVL_THEMES[theme[7:-3]] = import_module(f'bot.helper.themes.{theme[:-3]}')
...
def BotTheme(var_name, **format_vars):
    ...
    if theme_ in AVL_THEMES:
        text = getattr(AVL_THEMES[theme_].ZxZoneStyle(), var_name, None)
        if text is None:
        ...
        text = getattr(rantheme.ZxZoneStyle(), var_name, None)
        
    if text is None:
        text = getattr(zxzone_minimal.ZxZoneStyle(), var_name)
